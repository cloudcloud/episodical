defmodule Episodical.Workers.EpisodicalUpdate do
  ...
  defp update_episodic(
    ...
    date =
      case Date.from_iso8601(next_airing) do
        {:ok, date} ->
  ...
  defp capture_genres(
         {:ok, change, %{"data" => %{"id" => series_id, "genres" => genres}} = results},
         provider_id
       ) do
    genre_changes =
      Enum.map(genres, fn v ->
        %{
  ...
  defp capture_episodes(
         {:ok, change, %{"data" => %{"id" => series_id, "episodes" => episodes}} = results},
         provider_id
       ) do
    episode_changes =
      Enum.map(episodes, fn v ->
        released_at =
          try do
